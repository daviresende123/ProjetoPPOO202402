...
public class Veiculo {
        ...
        if (abastecendo) {
            ...
            if (colidiuComPosto(mapa, proximaLocalizacao)) {
                ...
                if (this instanceof Caminhao) {
                    setTempoParado(Duration.ofSeconds(30)); // 30 segundos para caminhões
                } else {
                    setTempoParado(Duration.ofSeconds(15)); // 15 segundos para carretos
                }
    ...
    private Localizacao encontrarUltimoVeiculoNaFila(Mapa mapa, Localizacao posicaoInicial) {
        ...
        while (true) {
            ...
            // Verifica se há um veículo na próxima posição
            for (Veiculo veiculo : mapa.getVeiculos()) {
            ...
            // Se não houver veículo na próxima posição, retorna a posição atual
            if (!encontrouVeiculo) {
                return posicaoAtual;
            }
        }
